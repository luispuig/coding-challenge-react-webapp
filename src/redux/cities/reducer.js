...
const reducer = (state:Array<_City> = initialState, action: _ActionPayload) => {

  switch (action.type) {
    case CITY_UPDATE__REQUEST:
      /* Reset state and temperature before fetch request */
      return state.map( item => (item.id === action.payload.id)? ({...item, state: 'request' }): item );

    case CITY_UPDATE__FAIL:
      /* Update Fail fields */
      if(action.payload.error === undefined) return state;
      return state.map( item => (item.id === action.payload.id)? ({...item, state: 'fail', error: action.payload.error }): item );

    case CITY_UPDATE__SUCCESS:
      /* Update Success fields */
      return state.map( item => (item.id === action.payload.id)? ({...item, state: 'success', temperature: action.payload.temperature}): item );
